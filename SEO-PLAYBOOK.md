...
| `PRICE-RANGE` | B2C roasted beans (187.5g bags): Rp 170.000–322.000. Slow-bar sessions have no fixed price — variable, depends on coffees available/ordered that day (Dan, 2026-07-17) | schema `priceRange` |
...
Acceptance:
- [x] `/faq/` + `/id/faq/` live, hreflang'd, in sitemap, linked from nav/footer. — shipped 2026-07-16
- [x] Every answer is self-contained (makes sense with zero surrounding context — the lift-out test).
- [x] FAQPage schema matches visible text 1:1; validates clean.

**Shipped 2026-07-16, facts corrected 2026-07-17.** 17 questions across 4 groups (Visiting & Reservations,
Buying the Beans, Wholesale & Export, About Roots & Acre & Indonesian Coffee),
generated by `scripts/build-pages.js` (`renderFAQ`, `FAQ_ITEMS`). Rendered as
plain `<h2>`+`<p>` — no accordion/JS — so full answer text is present in the
DOM for crawlers and AEO scrapers with no interaction needed. `FAQPage` +
`BreadcrumbList` schema added, reusing the homepage's `#business`/`#website`
`@id`s for entity consistency. Nav/footer/head became shared functions
(`renderNav`, `renderFooter`, `renderHeadCommon`) so the FAQ link now appears
site-wide at crawl depth 1, and the language toggle preserves the current
page across `/` ↔ `/id/` (previously always went to the home page).

One playbook-suggested question was **deliberately left out** rather than
answered with invented numbers (ground rule 1) — see SEO-TODO.md:
- "How fresh is the coffee when it ships?" — no confirmed roast-to-ship time.

Two facts corrected by Dan 2026-07-17: reservations are 1–4 pax per slot
(not "up to twelve" — the twelve-seat figure describes the venue, not a
single booking's size), and slow-bar session pricing has no fixed number —
